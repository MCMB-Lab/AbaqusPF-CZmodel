function Abaqus2PhasefieldUEL2DPlasticity(InputPath,MatProp)
...
% material for PFM in the Abasuqs CAE, which includes density, 24 SDV,
...
if narginLoc~=2
    ...
    for i=1:nMatIni
        ...
        MatProp(i,6)=input(['Thickness for material (' num2str(i) ')? - ']);
        MatProp(i,7)=input(['Density for material (' num2str(i) ')? - ']);
        MatProp(i,8)=input(['Split based on hydrostatic strain energy (0) or all strain energy (1) for material (' num2str(i) ')? - ']);
        MatProp(i,9)=input(['Von misses criterion is not used (0) or used (1) for material (' num2str(i) ')? - ']);
        MatProp(i,10)=input(['Length scale for material (' num2str(i) ')? - ']);
        MatProp(i,11)=input(['Critical energy release rate for material (' num2str(i) ')? - ']);
        MatProp(i,12)=input(['Linear elastic range is not used (0) or used (1) for material (' num2str(i) ')? - ']);
        MatProp(i,13)=input(['Damage (0) or redamage (1) switch for material (' num2str(i) ')? - ']);
    end
end
...
thick=MatProp(:,6);% Thickness for material
density=MatProp(:,7);% Density for material
anisoswt=MatProp(:,8);% Split based on hydrostatic strain energy (0) or all strain energy (1) for material
plswt=MatProp(:,9);% Von misses criterion is not used (0) or used (1) for material
lc=MatProp(:,10);% Length scale for material
gc=MatProp(:,11);% Critical energy release rate for material
elswt=MatProp(:,12);% Linear elastic range is not used (0) or used (1) for material
drswt=MatProp(:,13);% Damage (0) or redamage (1) switch for material
...
while(ischar(currentread))
    if contains(currentread,'*Element, type=') && ~contains(currentread,'COH')
        ...
        if length(str2num(currentread))==4
            % tetrahedron elements
            fprintf(fout1,['*********************** TRIANGULAR ****************************\n']);
            fprintf(fout1,['** ------------------- Displacement ---------------------------\n']);
            fprintf(fout1,['*User element, nodes=3, type=U1, properties=12, coordinates=2, VARIABLES=34\n']);
            fprintf(fout1,['1,2\n']);
            fprintf(fout1,['** -------------------- Phase  field --------------------------\n']);
            fprintf(fout1,['*User element, nodes=3, type=U3, properties=5, coordinates=2, VARIABLES=5\n']);
            fprintf(fout1,['3\n']);
            nNodeEle(1,CntEleTyp)=3;
        else
            % hexagonal elements
            fprintf(fout1,['**********************  RECTENGULAR  **************************\n']);
            fprintf(fout1,['** -------------------- Displacement --------------------------\n']);
            fprintf(fout1,['*User element, nodes=4, type=U2, properties=12, coordinates=2, VARIABLES=120\n']);
            fprintf(fout1,['1,2\n']);
            fprintf(fout1,['** -------------------- Phase  field --------------------------\n']);
            fprintf(fout1,['*User element, nodes=4, type=U4, properties=5, coordinates=2, VARIABLES=12\n']);
            fprintf(fout1,['3\n']);
...
nElemAll=0;
for ii=1:CntEleTyp
    if contains(SetNameEle{1,ii},'type=CPE3') || contains(SetNameEle{1,ii},'type=CPE4')
...
for ii=1:CntEleTyp
    if ~contains(SetNameEle{1,ii},'type=COH')
        ...
        fprintf(fout1,['***************************************************************\n']);
        if nNodeEle(1,ii)==3
            fprintf(fout1,['*Element, type=CPE3' '\n']);
        else
            fprintf(fout1,['*Element, type=CPE4' '\n']);
        end
        for jj=1:str2double(SetNameEle(3,ii))
        ...
        if nNodeEle(1,ii)==4
            fprintf(fout1,['***************************************************************\n']);
            fprintf(fout1,['*Element, type=COH2D4' '\n']);
            for jj=1:str2double(SetNameEle(3,ii))
                fprintf(fout1,[num2str(SetNameEle{2,ii}(jj,1)+2*nElemAll) ', ' num2str(SetNameEle{2,ii}(jj,2:end-1),'%d, ') ' ' num2str(SetNameEle{2,ii}(jj,end)) '\n']);
            end
        end
...
for k=1:nMat
    ...
    fprintf(fout1,[num2str(Ymod(nMatMatrix(k))) ', ' num2str(nu(nMatMatrix(k))) ', ' num2str(Ystress(nMatMatrix(k))) ', ' num2str(Hmod(nMatMatrix(k))) ...
        ', ' num2str(epscr(nMatMatrix(k))) ', ' num2str(thick(nMatMatrix(k))) ', ' num2str(density(nMatMatrix(k))) ', ' num2str(anisoswt(nMatMatrix(k))) ', '  '\n']);
    fprintf(fout1,[num2str(plswt(nMatMatrix(k))) ', ' num2str(lc(nMatMatrix(k))) ', ' num2str(gc(nMatMatrix(k))) ', ' num2str(drswt(nMatMatrix(k))) '\n']);
end
fprintf(fout1,['**E, nu, Y, H, eps_cr, thick, rho, anisoswt \n']);
fprintf(fout1,['**plswt, lc, gc, drswt \n']);
fprintf(fout1,['***************************************************************\n']);
...
for ii=1:nMat
    ...
    fprintf(fout1,[num2str(lc(ii)) ', ' num2str(gc(ii)) ', ' num2str(thick(k)) ', '  num2str(elswt(ii)) ', ' num2str(drswt(ii)) '\n']);
end
fprintf(fout1,['**lc, gc, thick, elswt, drswt\n']);
...
if isempty(fullp)~=1
    input2=[fullp(1:end-33) '\PhaseFieldUEL\Abaqus2PhasefieldUEL2D.for'];
else
    fullp=matlab.desktop.editor.getActiveFilename;
    input2=[fullp(1:end-35) '\PhaseFieldUEL\Abaqus2PhasefieldUEL2D.for'];
end
